...
func TestBuildTrackerURL(t *testing.T) {
	...
	assert.Nil(t, err)
	assert.Equal(t, url, expected) // revisamos la URL
}

// test a los peers realizando una solicitud http de prueba
func TestRequestPeers(t *testing.T) {
	// creamos un servidor de prueba
	ts := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		// respondemos en binario bittorrent
		response := []byte(
			"d" +
				"8:interval" + "i900e" +
				"5:peers" + "12:" +
	...
	// revisamos las direcciones obtenidas
	assert.Nil(t, err)
